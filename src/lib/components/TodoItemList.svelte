<script lang="ts">
	...
	async function fetchTodos(currentFilter: Filter = filter, currentPage: number = page) {
		...
		const result = await listTodosInList({
			path: { 'list-id': listId },
			query: { completed, page: currentPage }
		});
		if (result.data) {
	...
	async function handleCreate(event: SubmitEvent) {
		...
		if (!name) return;
		const result = await createTodoInList({ path: { 'list-id': listId }, body: { name } });
		if (result.data) {
			newTodoName = '';
			await fetchTodos();
		}
	...
	async function handleToggle(todo: TodoResponse) {
		await updateTodoInList({
			path: { 'list-id': listId, 'todo-id': todo.id },
	...
	async function handleDelete(id: string) {
		await deleteTodoInList({ path: { 'list-id': listId, 'todo-id': id } });
		await fetchTodos();
	}
